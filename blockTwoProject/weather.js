    ...
    .then(data => {
        ...
        data.hourly.forEach(element => {
            ...
            <div class="data-card">
                ...
                <div>${kelvinToFaherenheit(element.temp)}&#176F</div>
                <div>${pascalToMercury(element.pressure)}in.</div>
                <div>${tempDifference(data.current.temp, element.temp)}&#176F</div>
                <div>${pressureDifference(data.current.pressure, element.pressure)}in.</div>
            </div>
            `;
        });
    })
